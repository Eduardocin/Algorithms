...
public:
    ...
    tuple<int, V*> find(const K& key){
        int h = hash(key);
        int pos = h;
        if(table[pos].state == 1 && table[pos].key == key){
            return make_tuple(pos, &table[pos].value);
        }
        else{
...
int main(){
    ...
    while(cin >> tam_arr_perm && tam_arr_perm!= 0){
        ...
        for(int i = 0; i < num_operations; i++){
            ...
            if(command == "find"){
                cin >> key;
                tuple<int, int*> result = table.find(key);
                int index = get<0>(result);
                int* val_ptr = get<1>(result);
                if(index != -1){
                    cout << index << " " << *val_ptr << endl;
                }
                else{
                    cout << -1 << endl;
                }
